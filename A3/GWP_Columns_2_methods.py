...
#-------------Choose GWP-display method----------------------------------------------------------------------------------

#Method 2, Adjusted GWP
...
#------------------------------------Retrieve Column Data from Ifc File---------------------------------------------------------------------
    ...
    for (type_name, material, volume, height), data in info_dict.items():
        if 'Concrete' in material:
            ...
            if 'Column' in type_name:
                ...
                # Assuming all non-column concrete elements have a reinforcement density of 100
                reinforcement_volume = data['volume'] * 100
...
#--------------------------------------------------Mode 1: Real GWP----------------Add Reinforcement GWP to Parent GW------------------------------------------------------
...
#----------------------------------------------------------------Calculate Avarage GWP for lowest and highest Storey---------------------------------------------------
...
# Define `level_numbers` based on `column_levels_by_z`
...
# Initialize list to store each column's individual adjusted GWP data
...
def create_color_gradient(gwp_values):
    ...
    color_map = colormaps.get_cmap("RdYlGn_r")  # Reverse for red high, green low

    # Generate colors for each normalized value
    colors = [color_map(norm_value) for norm_value in normalized_values]
    return colors

# Collect GWP values for normalization based on type, height, and level as unique keys
gwp_values = []
unique_column_keys = []
...
# Generate color gradient based on normalized GWP values
colors = create_color_gradient(gwp_values)

# Assign colors to each column entry based on the unique key (type, height, level)
...
# Dictionary to hold unique colors for each combination of ObjectType and rounded height
type_colors = {}

# Create a main Base object to hold all columns
...
# Assign levels based on exact z-coordinates of objects
def get_column_levels_by_exact_z(columns):
    level_dict = {}
    z_coordinates = []
...
# Get column levels based on z-coordinate ordering
column_levels_by_z = get_column_levels_by_exact_z(columns)

# Dictionary to hold unique colors for each combination of ObjectType, height, and level
predefined_colors = {
    (item['type_name'], item['height'], item['level']): item['color'] for item in individual_adjusted_gwp_list
}


#------------------------------------------------------Colour Code columns for speckle----------------------------------------------------

# Helper function to convert RGB to Hexadecimal
def rgb_to_hex(rgb):
    return (int(rgb[0] * 255) << 16) + (int(rgb[1] * 255) << 8) + int(rgb[2] * 255)

# Function to generate White color for non identilyed elements
def generate_random_color():
    return 0xFFFFFF  # Hexadecimal for white
...
# Send all columns as one object to Speckle
